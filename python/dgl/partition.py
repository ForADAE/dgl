...
def metis_partition_assignment(
    ...
    * `balance_edges` balances the number of edges in each partition.
    ...
    node_part = _CAPI_DGLMetisPartition_Hetero(
        sym_g._graph, k, vwgt, mode, (objtype == objtype)
    )
    print(
...
def metis_partition_assignment_halo(
    g, k, balance_ntypes=None, balance_edges=False, mode="k-way", objtype="cut", tpwgts=None
):
    """This assigns nodes to different partitions with Metis partitioning algorithm.
    ...
    * `balance_edges` balances the number of edges in each partition.
    ...
    start = time.time()
    # print(tpwgts)
    # tpwgts = [F.tensor([x]) if isinstance(x, float) else x for x in tpwgts]
    # tpwgts = F.stack(tpwgts, 1)
    # shape = (
    #     np.prod(
    #         F.shape(tpwgts),
    #     ),
    # )
    # tpwgts = F.reshape(tpwgts, shape)
    # tpwgts = F.to_dgl_nd(tpwgts)
    # print(tpwgts)

    node_part = _CAPI_DGLMetisPartition_Hetero_halo(
...
def metis_partition_assignment_weight(
    g, k, balance_ntypes=None, balance_edges=False, mode="k-way", objtype="cut", tpwgts=None
):
    """This assigns nodes to different partitions with Metis partitioning algorithm.
    ...
    * `balance_edges` balances the number of edges in each partition.
    ...
    node_part = _CAPI_DGLMetisPartition_Hetero_weight(
        sym_g._graph, k, vwgt, mode, (objtype == "cut"), tpwgts
    )
    print(
        "Metis partitioning: {:.3f} seconds, peak memory: {:.3f} GB".format(
            time.time() - start, get_peak_mem()
