...
IdArray MetisPartition_halo(
  ...
  auto partition_func = METIS_PartGraphRecursive_halo;
  // auto partition_func =
  //     (mode == "k-way") ? METIS_PartGraphKway_halo : METIS_PartGraphRecursive_halo;
  ...
  printf("METIS_PartGraphKway_halo\n");

  int ret = METIS_PartGraphKway_halo(
      &nvtxs,  // The number of vertices
      &ncon,   // The number of balancing constraints.
      xadj,    // indptr
      adjncy,  // indices
...
DGL_REGISTER_GLOBAL("partition._CAPI_DGLMetisPartition_Hetero_halo")
    .set_body([](DGLArgs args, DGLRetValue *rv) {
      HeteroGraphRef g = args[0];
      auto hgptr = std::dynamic_pointer_cast<HeteroGraph>(g.sptr());
      CHECK(hgptr) << "Invalid HeteroGraph object";
      ...
      bool obj_cut = args[4];
#if !defined(_WIN32)
      *rv = MetisPartition_halo(ugptr, k, vwgt, mode, obj_cut);
#else
      LOG(FATAL) << "Metis partition does not support Windows.";
#endif  // !defined(_WIN32)
    });
...
IdArray MetisPartition_weight(
    UnitGraphPtr g, int k, NDArray vwgt_arr, const std::string &mode,
    bool obj_cut, NDArray tpwgts_arr) {
  // Mode can only be "k-way" or "recursive"
  CHECK(mode == "k-way" || mode == "recursive")
      << "mode can only be \"k-way\" or \"recursive\"";
  // The index type of Metis needs to be compatible with DGL index type.
  ...
  int64_t tpwgts_len = tpwgts_arr->shape[0];
  CHECK_EQ(sizeof(idx_t), tpwgts_arr->dtype.bits / 4)
      << "The tpwgts array doesn't have right type";
  float *tpwgts = NULL;
  if (tpwgts_len > 0) {
    tpwgts = static_cast<float *>(tpwgts_arr->data);
  }
  ...
  int ret = partition_func(
      ...
      NULL,     // The weights of the edges
      &nparts,  // The number of partitions.
      tpwgts,     // the desired weight for each partition and constraint
      NULL,     // the allowed load imbalance tolerance
      options,  // the array of options
      &objval,  // the edge-cut or the total communication volume of
      // the partitioning solution
...
DGL_REGISTER_GLOBAL("partition._CAPI_DGLMetisPartition_Hetero_weight")
    .set_body([](DGLArgs args, DGLRetValue *rv) {
      HeteroGraphRef g = args[0];
      auto hgptr = std::dynamic_pointer_cast<HeteroGraph>(g.sptr());
      CHECK(hgptr) << "Invalid HeteroGraph object";
      ...
      std::string mode = args[3];
      bool obj_cut = args[4];
      NDArray tpwgts = args[5];
#if !defined(_WIN32)
      *rv = MetisPartition_weight(ugptr, k, vwgt, mode, obj_cut, tpwgts);
#else
      LOG(FATAL) << "Metis partition does not support Windows.";
#endif  // !defined(_WIN32)
    });
